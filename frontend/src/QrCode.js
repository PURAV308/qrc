...
const QrCode = () => {
  ...
  useEffect(() => {
    const fetchScannedData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/qrcodes/all'); // Replace with your backend endpoint
  ...
  const saveScannedDataToBackend = async (scannedText) => {
    try {
      await axios.post('http://localhost:5000/api/qrcodes/add', {
        text: scannedText,
        confirmed: false,
      });
